...
```{r}
target = c('Attrition')
model = c('BusinessTravel','Department','EducationField', 'OverTime', 'Gender', 'HourlyRate', 'JobInvolvement', 'JobLevel')
target_string = glue::glue("{target} ~ ")
f = target_string + paste(model, collapse="+")

sample = select(attrition_data, model, target)
str(sample)
summary(sample)

#Train Test Split
seed = 1234
sample_size = floor(0.75*nrow(sample))
set.seed(seed)
