...
```{r}
sample = select(attrition_data, BusinessTravel,Department,EducationField,Attrition, OverTime)
str(sample)
summary(sample)

#Train Test Split
seed = 1234
sample_size = floor(0.5*nrow(sample))
set.seed(seed)
...
summary(test)
#Build the model
linear_model <-vglm(Attrition~BusinessTravel+Department+EducationField+OverTime,family = "multinomial",data=train)

#Summarize the model
summary(linear_model)

#Run Predictions
x<-select(test, -Attrition)
y<-select(test, Attrition)

probability<-predict(linear_model,x,type="response")
test$pred_log_reg<-apply(probability,1,which.max)
test$pred_log_reg[which(test$pred_log_reg=="1")]<-levels(test$Attrition)[1]
test$pred_log_reg[which(test$pred_log_reg=="2")]<-levels(test$Attrition)[2]



#Accuracy of the model
mtab<-table(test$pred_log_reg,test$Attrition)
